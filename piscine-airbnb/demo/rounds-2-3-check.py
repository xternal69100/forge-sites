...
SITE=ROOT.parent
PAGES={name:ROOT/name for name in ("index.html","member.html","host.html","admin.html")}
...
def main()->int:
    ...
    combined="\n".join(sources.values())
    for source in sources.values(): require(source,KEY,DISCLOSURE)
    require(sources["index.html"],"Proposer ma piscine","member.html","host.html","admin.html")
    require(sources["host.html"],*STATES,"hostApplications","eventJournal","appendEvent","example.test","documents","securityChecklist","visitSlot","published")
    require(sources["member.html"],"Portefeuille","Préférences","Réservations","Parrainage","Invitation locale","example.test")
