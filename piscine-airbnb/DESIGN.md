...
- **Règle tarifaire transversale** : toutes les surfaces utilisent le total standard `G` du groupe/créneau avant promotion, inchangé par rapport aux totaux `17-pilote`. La **commission Poolbnb 25 % incluse** vaut `round_half_up(25 % × G)` et le net hôte reçoit le résiduel `G − commission`. Aucune ligne additive n’apparaît au checkout. Le pack suit exclusivement `G → −20 % → −CHF 20 → total client`, sans réduire le net hôte ni la commission contractuelle. Caution remboursable, taxes et débours qualifiés restent séparés et hors assiette. Les coûts inconnus sont `null / À mesurer`, jamais zéro ; la politique des nouvelles réservations est `gross_25_included_v1`.
