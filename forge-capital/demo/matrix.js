...
function traceMarkup(item){return `<article class="trace-row trace-${esc(item.classification.toLowerCase().replace(/[^a-z]+/g,'-'))}"><span class="trace-seq">${String(item.sequence).padStart(2,'0')}</span><span class="trace-class">${esc(item.classification)}</span><span class="trace-role">${esc(item.role)}</span><span class="trace-method">${esc(item.method)}</span><span class="trace-status">${esc(item.status)}</span><span class="trace-message">${esc(item.message)}</span><span class="trace-source">${esc(item.source)}</span></article>`;}
...
async function loadSnapshot(){
  try{
    ...
    $('#agent-source-health').textContent=`Collecte différée ${snapshot.agentCell.sourceSummary.httpSuccess}/${snapshot.agentCell.sourceSummary.expected} HTTP ; elle n’implique ni edge ni performance.`;
